    ...
    <div className="space-y-4 smps-fade-in">
      ...
      <div className="space-y-2">
        {sectionProgress.map(({ section, label, questions: sectionQs, answered, total, pct }, idx) => {
          ...
          return (
            <div key={section} className={`rounded-lg border transition-[background-color,border-color] duration-200 ease-out ${isOpen ? 'bg-card' : 'bg-card hover:bg-card/80'}`}>
                          ...
                          return (
                            <div key={q.id} className="rounded-md border bg-background/50 px-4 py-3">
                              ...
                              <div className="flex gap-1.5 flex-wrap">
                                <button onClick={() => handleNA(q.id)} className={`smps-score-btn min-w-[72px] py-1.5 px-2 rounded-md text-xs font-medium border transition-[background-color,border-color,color,transform,box-shadow] duration-150 ease-out flex items-center justify-center gap-1 ${isNA ? 'bg-foreground/10 text-foreground border-foreground/20' : 'bg-muted/50 text-muted-foreground border-transparent hover:border-border'}`}>
                                  <Ban className="h-3 w-3" /> No Aplica
                                </button>
                                {[1, 2, 3, 4, 5].map(score => (
                                  <button key={score} onClick={() => handleScore(q.id, score)} className={`smps-score-btn flex-1 min-w-[60px] py-1.5 px-2 rounded-md text-xs font-medium border transition-[background-color,border-color,color,transform,box-shadow] duration-150 ease-out ${!isNA && responses[q.id] === score ? 'bg-accent text-accent-foreground border-accent shadow-sm' : 'bg-muted/50 text-muted-foreground border-transparent hover:border-border'}`}>
                                    {SCORE_LABELS[score]}
                                  </button>
                                ))}
                              </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <button onClick={() => setShowConfirm(true)} disabled={!canSubmit} className="ml-auto px-5 py-2 rounded-md bg-accent text-accent-foreground text-sm font-semibold disabled:opacity-40 hover:opacity-90 transition-[opacity,transform] duration-150 ease-out active:scale-[0.98]">Guardar Autoevaluación</button>
      </div>
