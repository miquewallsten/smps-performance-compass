...
export default function SelfEvaluation() {
    ...
    return (
      <div className="space-y-4 smps-fade-in">
        ...
        <div className="smps-surface-elevated text-center py-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">Tu calificación: {existing?.totalScore ?? submitted ? 'Enviada' : '—'}%</p>
        </div>
      </div>
    );
  }

  const sectionProgress = sectionGroups.map(g => {
    const answered = g.questions.filter(q => responses[q.id] !== undefined || naQuestions[q.id]).length;
    return { ...g, answered, total: g.questions.length, pct: Math.round((answered / g.questions.length) * 100) };
  });
  ...
  return (
    <div className="space-y-4 smps-fade-in">
      ...
      </div>      <div className="smps-surface-card">
