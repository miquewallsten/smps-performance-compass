...
/* ═════════════════════════════════════════════════════════════════════════════
   ...
   Light theme · Full viewport · Horizontal scroll · Vertical mini-viewer
   ═════════════════════════════════════════════════════════════════════════════ */
...
export default function UserTimeline() {
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      {/* ═══ HEADER ═══════════════════════════════════════════════════════════ */}
      <div className="shrink-0 border-b bg-background px-5 py-3 flex items-center gap-3">
        <button onClick={() => nav(-1)} className="p-1.5 rounded-md hover:bg-muted transition-[background-color] 150ms ease-out text-muted-foreground hover:text-foreground">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-muted-foreground">{total} evento{total!==1?'s':''} registrado{total!==1?'s':''}</p>
        </div>

        {/* Filter dropdown */}
        <div ref={filterRef} className="relative">
          <button onClick={() => setShowFilter(!showFilter)}
            className={`h-7 px-2.5 rounded-md flex items-center gap-1.5 text-xs font-medium transition-[background-color,color,box-shadow] 150ms ease-out ${
      ...
      {/* ═══ INLINE NOTE ═════════════════════════════════════════════════════ */}
      {showNote && (
        <div className="shrink-0 border-b px-5 py-2.5 bg-muted/30">
          <div className="flex items-center gap-2">
            <MessageSquare className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
            <input value={noteText} onChange={e => setNoteText(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && addNote()}
              placeholder="Nota para el historial..."
              className="flex-1 min-w-0 h-7 px-2.5 rounded-md text-xs border bg-background focus:outline-none focus:ring-1 focus:ring-accent transition-[border-color,box-shadow] 150ms ease-out"
              autoFocus />
            <button onClick={addNote} disabled={!noteText.trim()}
              className="h-7 px-3 rounded-md text-xs font-semibold bg-accent text-accent-foreground hover:opacity-90 disabled:opacity-30 transition-[opacity] 150ms ease-out active:scale-[0.97]">
              Guardar
            </button>
            <button onClick={() => setShowNote(false)} className="p-1 text-muted-foreground hover:text-foreground transition-[color] 150ms ease-out">
      ...
      {/* ═══ YEAR NAV ════════════════════════════════════════════════════════ */}
      {years.length > 0 && (
        <div className="shrink-0 border-b px-5 py-1.5 flex items-center gap-2 bg-background">
      ...
      <div className="shrink-0 h-px bg-border">
        <div className="h-full bg-accent/50 transition-[width] 200ms ease-out" style={{ width: `${scrollPct*100}%` }} />
      </div>
        ...
        <div className="flex-1 min-w-0 relative overflow-hidden">
          ...
          {scrollPct > 0.02 && (
            <button onClick={() => scrollJump(-1)}
              className="absolute left-0 top-0 bottom-0 w-8 z-10 flex items-center justify-center bg-gradient-to-r from-background via-background/80 to-transparent tm-arr">
              <ChevronLeft className="h-4 w-4 text-muted-foreground" />
            </button>
          )}
          {scrollPct < 0.98 && events.length > 3 && (
            <button onClick={() => scrollJump(1)}
              className="absolute right-0 top-0 bottom-0 w-8 z-10 flex items-center justify-center bg-gradient-to-l from-background via-background/80 to-transparent tm-arr">
          ...
          {isLoading ? (
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                ...
                {filter !== 'all' && (
                  <button onClick={() => setFilter('all')} className="mt-2 text-xs text-accent hover:underline">Ver todo</button>
                )}
              </div>
            </div>
          ) : (
            <div ref={scrollRef} className="h-full overflow-x-auto overflow-y-hidden tm-scroll">
              <div className="flex items-stretch min-h-full pt-8 pb-12 px-6">
                {/* Origin dot */}
                <div className="shrink-0 w-6 flex items-center justify-center">
                  <div className="w-2 h-2 rounded-full bg-accent tm-glow-dot" />
                </div>
                  ...
                  return (
                    <div key={mk} data-m={mk} className="shrink-0 flex flex-col items-center tm-month" style={{ animationDelay: `${mi*60}ms` }}>
                      ...
                      <div className="shrink-0 mb-3 text-center h-6 flex items-end">
                        {isFirstYear && (
                          <span className="text-lg font-bold text-foreground/15 mr-1.5 leading-none" style={{ fontFamily: 'var(--font-display)' }}>{ys}</span>
                        )}
                        <span className={`text-[10px] uppercase tracking-widest font-semibold ${isFirstYear ? 'text-accent' : 'text-muted-foreground'}`} style={{ fontFamily: 'var(--font-display)' }}>
                          {monthLabel(mk)}
                        </span>
                      </div>
                          ...
                          {me.map((ev: any, ei: number) => {
                            ...
                            return (
                              <div key={ev.id} className="relative tm-node" style={{ animationDelay: `${mi*60+ei*40}ms` }}>
                                {/* Connector line */}
                                <div className={`absolute top-1/2 -translate-y-1/2 h-px w-4 ${above?'right-1/2':'left-1/2'}`}
                                  style={{ backgroundColor: cfg.accent, opacity: 0.25 }} />

                                {/* Rail node */}
                                <div className="relative z-10 flex items-center justify-center w-6 h-6">
                                  <div className="w-2.5 h-2.5 rounded-full border-2 bg-background tm-dot"
                                    style={{ borderColor: cfg.accent, boxShadow: `0 0 0 3px ${cfg.accent}12` }} />
                                </div>
                                ...
                                <div className={`absolute ${above?'bottom-7':'top-7'} left-1/2 -translate-x-1/2`}>
                                  <div className={`w-56 rounded-lg border bg-card cursor-pointer overflow-hidden tm-card ${isExp?'tm-card-exp':''}`}
                                    ...
                                    <div className="px-3 py-2">
                                      ...
                                      {cl && (
                                        <span className="inline-block text-[9px] font-medium px-1.5 py-0.5 rounded bg-accent/8 text-accent">{cl}</span>
                                      )}
                                          ...
                                          {ev.event_type === 'evaluation_completed' && (
                                            <div className="space-y-1">
                                              ...
                                              {meta.score !== undefined && (
                                                <div className="flex items-center gap-2">
                                                  <Ring v={meta.score} /><span className="text-sm font-bold text-accent">{meta.score}%</span>
                                                </div>
                                              )}
                                            </div>
                                          )}
                                          ...
                                          {ev.event_type === 'role_change' && meta.changes && (
                                            <div className="flex flex-wrap gap-1">
                                              {(meta.changes as string[]).map((c:string,i:number) => (
                                                <span key={i} className="text-[9px] bg-muted px-1.5 py-0.5 rounded font-medium">{c}</span>
                                              ))}
                                            </div>
                                          )}

                                          {(ev.event_type==='supervisor_assigned'||ev.event_type==='supervisor_removed') && meta.supervisorName && (
                                            <p className="text-xs"><span className="font-medium">{meta.supervisorName}</span>
                                            {meta.period && <span className="text-muted-foreground text-[11px] ml-1">{meta.period}</span>}</p>
                                          )}
          ...
          <div className="shrink-0 w-12 border-l bg-muted/30 flex flex-col">
                ...
                return (
                  <div key={mk} className="flex flex-col items-center">
                    ...
                    <button onClick={() => jumpToMonth(mk)}
                      ...
                      <span className="text-[8px] text-muted-foreground/60 group-hover:text-foreground transition-[color] 100ms ease-out leading-none">
                        {monthLabel(mk)}
                      </span>
                    </button>
                  </div>
            ...
            <div className="shrink-0 mx-1.5 mb-1.5 h-1 rounded-full bg-border overflow-hidden">
              <div className="h-full rounded-full bg-accent/60 transition-[width,margin] 200ms ease-out"
                style={{
                  width: `${Math.max(15, 100 / Math.max(1, sortedMonths.length))}%`,
                  marginLeft: `${scrollPct * (100 - 100 / Math.max(1, sortedMonths.length))}%`
                }} />
            </div>
          </div>
        )}
