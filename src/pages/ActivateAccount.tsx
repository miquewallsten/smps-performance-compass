...
export default function ActivateAccount() {
  ...
  useEffect(() => {
    ...
    api.get<{ email?: string; name?: string; error?: string }>(`/api/auth/verify-activation?token=${encodeURIComponent(token)}`)
      .then((data) => {
        setValidToken(true);
        setUserName(data.name || '');
      })
      .catch((err) => {
        setError(err.message || 'Token de activación inválido o expirado.');
        setValidToken(false);
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post('/api/auth/activate', { token, password, confirmPassword });
      setActivated(true);
    } catch (err: any) {
  ...
  if (verifying) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        ...
        <div className="relative z-10 text-white text-lg">Verificando token de activación...</div>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-background">
        ...
        <div className="relative z-10 w-full max-w-md px-6">
          <div className="bg-card rounded-xl shadow-2xl p-8 text-center">
            ...
            <h1 className="font-display text-2xl font-bold text-foreground mb-2">¡Cuenta Activada!</h1>
            <p className="text-muted-foreground text-sm mb-6">Su cuenta ha sido activada exitosamente. Ya puede iniciar sesión.</p>
      ...
      <div className="min-h-screen flex items-center justify-center bg-background">
        ...
        <div className="relative z-10 w-full max-w-md px-6">
          <div className="bg-card rounded-xl shadow-2xl p-8 text-center">
            ...
            <h1 className="font-display text-2xl font-bold text-foreground mb-2">Token Inválido</h1>
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        <div className="bg-card rounded-xl shadow-2xl p-8">
          <div className="text-center mb-6">
            ...
            <h1 className="font-display text-2xl font-bold text-foreground">Activar Cuenta</h1>
            {userName && <p className="text-muted-foreground text-sm mt-1">Bienvenido, {userName}</p>}
          </div>
