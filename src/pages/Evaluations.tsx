...
export default function Evaluations() {
  ...
  if (selectedEmployee && !submitted) {
    ...
    const handleSubmit = () => {
      if (!canSubmit) return;
      const evalResponses = [
        ...Object.entries(responses).map(([questionId, score]) => ({ questionId, score, notApplicable: false, noElements: false, weight: Math.round(questions.find(q => q.id === questionId)?.weight || 1) })),
        ...Object.keys(naQuestions).map(questionId => ({ questionId, score: 0, notApplicable: true, noElements: false, weight: Math.round(questions.find(q => q.id === questionId)?.weight || 1) })),
        ...Object.keys(noElementsQuestions).map(questionId => ({ questionId, score: 0, notApplicable: false, noElements: true, weight: Math.round(questions.find(q => q.id === questionId)?.weight || 1) })),
      ];
  ...
  return (
    <div>
      ...
      {pendingEmployees.length === 0 && completedEmployees.length === 0 && !isAdminOrSocio && viewableUsers.length === 0 && (
        <div className="text-center py-12 text-muted-foreground">
          <p>No tienes evaluaciones asignadas para este periodo.</p>
        </div>
      )}
