...
export default function Layout() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-6">
        <div className="smps-surface-elevated max-w-sm text-center">
          ...
          <button onClick={() => { logout(); navigate('/login'); }} className="px-5 py-2 rounded-md bg-accent text-accent-foreground text-sm font-medium hover:opacity-90 transition-all duration-150 active:scale-[0.98]">Cerrar Sesión</button>
        </div>
      </div>
    );
  }
  ...
  const evalItems = [
    ...
    { to: '/evaluations', icon: ClipboardList, label: 'Evaluar', show: hasTeam || isAdminOrSuper },
  ];
        ...
        <aside className={`${collapsed ? 'w-14' : 'w-52'} bg-sidebar border-r border-sidebar-border transition-all duration-200 flex-shrink-0 hidden md:flex flex-col fixed top-14 left-0 bottom-0 z-40`}>
          <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto">
            {showEvalModule && (
              <>
                {!collapsed && (
                  <button onClick={() => setEvalGroupOpen(!evalGroupOpen)} className="flex items-center gap-2 px-3 py-1.5 text-[11px] uppercase tracking-widest text-sidebar-foreground/40 font-semibold w-full hover:text-sidebar-foreground/60 transition-colors">
                ...
                {(evalGroupOpen || collapsed) && evalItems.map(item => <div key={item.to} className="relative">{renderNavItem(item)}</div>)}
                {!collapsed && <div className="border-b border-sidebar-border my-1.5" />}
              </>
            )}
            {otherItems.map(item => <div key={item.to} className="relative">{renderNavItem(item)}</div>)}
          </nav>
