...
// ─── GET /api/users ──────────────────────────────────────────────────────
router.get('/', authMiddleware, requireAdmin, (_req: Request, res: Response) => {
  try {
...
router.get('/:id', authMiddleware, requireSelfOrAdmin, (req: Request, res: Response) => {
  try {
    const user = db.prepare(`SELECT ${SAFE_USER_COLUMNS} FROM users WHERE id = ?`).get(req.params.id) as Record<string, unknown> | undefined;
...
router.post('/', authMiddleware, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const user = db.prepare(`SELECT ${SAFE_USER_COLUMNS} FROM users WHERE id = ?`).get(id) as Record<string, unknown>;
...
// ─── PATCH /api/users/:id ────────────────────────────────────────────────
router.patch('/:id', authMiddleware, requireSelfOrAdmin, (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (isAdminUser) {
      ...
      const { name, email, position, practiceArea, customPositionId, isActive } = req.body as {
        name?: string;
        email?: string;
        position?: string;
        practiceArea?: string;
    ...
    const updatedUser = db.prepare(`SELECT ${SAFE_USER_COLUMNS} FROM users WHERE id = ?`).get(id) as Record<string, unknown>;
...
router.patch('/:id/role', authMiddleware, requireAdmin, (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (isManagingPartner !== undefined) { updates.push('is_managing_partner = ?'); values.push(isManagingPartner ? 1 : 0); }
