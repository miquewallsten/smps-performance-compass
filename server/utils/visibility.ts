...
export function canViewUserEvaluations(viewer: any, target: any): boolean {
  if (viewer.id === target.id) return true;
  if (viewer.is_super_user || viewer.is_admin) return true;
  if (viewer.is_managing_partner) return true;
  if (viewer.position === 'socio') {
    if (target.is_managing_partner) return false;
...
export function filterVisibleUsers(viewer: any, users: any[]): any[] {
