  ...
  `);
    const transaction = db.transaction(() => {
        ...
        for (const user of USERS) {
            ...
            insertUser.run(id, user.email, passwordHash, securityQuestion, securityAnswerHash, user.name, user.position, user.practiceArea || null, user.isAdmin ? 1 : 0, 0, // isSuperUser - only lab@bowdot.com is SuperUser, already in DB
            user.isManagingPartner ? 1 : 0, user.isActive ? 1 : 0, mustChange);
        ...
        for (const assignment of ASSIGNMENTS) {
            const employee = db.prepare('SELECT id FROM users WHERE email = ?').get(assignment.employeeEmail);
            const supervisor = db.prepare('SELECT id FROM users WHERE email = ?').get(assignment.supervisorEmail);
    ...
    const assignmentCount = db.prepare('SELECT COUNT(*) as count FROM supervisor_assignments').get().count;
