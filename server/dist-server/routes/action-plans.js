...
router.post('/', authMiddleware, (req, res) => {
    try {
        ...
        const insertPlan = db.prepare(`INSERT INTO action_plans (id, employee_id, supervisor_id, period, content, approval_status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)`);
        const insertItem = db.prepare(`INSERT INTO smart_action_items (id, action_plan_id, competencia, objetivo, acciones, que_evitar, fecha_revision, apoyos)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
...
router.patch('/:id', authMiddleware, (req, res) => {
    try {
        ...
        if (items && Array.isArray(items)) {
            ...
            const insertItem = db.prepare(`INSERT INTO smart_action_items (id, action_plan_id, competencia, objetivo, acciones, que_evitar, fecha_revision, apoyos) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
...
//# sourceMappingURL=action-plans.js.map