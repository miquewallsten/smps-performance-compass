...
router.get('/', authMiddleware, (req, res) => {
    try {
        ...
        const result = announcements.map((a) => {
            const readBy = db.prepare('SELECT user_id FROM announcement_reads WHERE announcement_id = ?').all(a.id).map((r) => r.user_id);
            return { ...a, readBy };
        }).filter((a) => {
...
router.post('/', authMiddleware, requireAdmin, (req, res) => {
    try {
        ...
        if (!['all', 'legal', 'administrativo'].includes(audience))
...
router.patch('/:id', authMiddleware, requireAdmin, (req, res) => {
    try {
        ...
        const readBy = db.prepare('SELECT user_id FROM announcement_reads WHERE announcement_id = ?').all(req.params.id).map((r) => r.user_id);
