...
function fetchResponses(evaluationIds) {
    ...
    const map = new Map();
    for (const r of rows) {
        if (!map.has(r.evaluation_id))
            map.set(r.evaluation_id, []);
        map.get(r.evaluation_id).push(r);
    }
    return map;
}
// Helper: fetch NA approvals for given evaluation IDs
function fetchNaApprovals(evaluationIds) {
    if (evaluationIds.length === 0)
        return new Map();
    const placeholders = evaluationIds.map(() => '?').join(',');
...
router.get('/', authMiddleware, (req, res) => {
    try {
        ...
        if (evaluatedId) {
            sql += ' AND evaluated_id = ?';
            params.push(evaluatedId);
        }
        if (type) {
            sql += ' AND type = ?';
            params.push(type);
...
router.post('/', authMiddleware, (req, res) => {
    try {
        ...
        const totalScore = respArr.length > 0 ? respArr.reduce((sum, r) => sum + (r.score || 0), 0) / respArr.length : 0;
        const insertEval = db.prepare(`INSERT INTO evaluations (id, evaluator_id, evaluated_id, period, type, comments, supervisor_comments, total_score, completed_at, feedback_completed, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)`);
        const insertResp = db.prepare(`INSERT INTO evaluation_responses (id, evaluation_id, question_id, score, not_applicable, no_elements)
       VALUES (?, ?, ?, ?, ?, ?)`);
...
router.patch('/:id', authMiddleware, (req, res) => {
    try {
        ...
        if (responses && Array.isArray(responses)) {
            ...
            const insertResp = db.prepare(`INSERT INTO evaluation_responses (id, evaluation_id, question_id, score, not_applicable, no_elements) VALUES (?, ?, ?, ?, ?, ?)`);
...
router.patch('/:id/feedback', authMiddleware, (req, res) => {
    try {
        const evaluation = db.prepare('SELECT * FROM evaluations WHERE id = ?').get(req.params.id);
        if (!evaluation)
            return res.status(404).json({ error: 'Evaluation not found' });
...
router.patch('/:id/na-approval', authMiddleware, (req, res) => {
    try {
        const evaluation = db.prepare('SELECT * FROM evaluations WHERE id = ?').get(req.params.id);
        if (!evaluation)
            return res.status(404).json({ error: 'Evaluation not found' });
