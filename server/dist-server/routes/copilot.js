...
router.get('/config', (_req, res) => {
    try {
        let config = db.prepare('SELECT * FROM copilot_config WHERE id = 1').get();
        if (!config) {
            db.prepare(`
      ...
      `).run();
            config = db.prepare('SELECT * FROM copilot_config WHERE id = 1').get();
        }
...
router.get('/conversations/:id', (req, res) => {
    try {
        const conversation = db.prepare('SELECT * FROM copilot_conversations WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
        if (!conversation)
...
// ─── SYSTEM PROMPT ────────────────────────────────────────────────────────────
function buildSystemPrompt(config, userId) {
    // Gather live system context
    const userCount = db.prepare('SELECT COUNT(*) as c FROM users WHERE is_active = 1').get().c;
    const adminCount = db.prepare('SELECT COUNT(*) as c FROM users WHERE is_admin = 1 AND is_active = 1').get().c;
    const currentPeriods = db.prepare('SELECT * FROM period_configs ORDER BY period DESC').all();
    const modules = db.prepare('SELECT * FROM module_config WHERE id = 1').get();
    const systemStatus = db.prepare('SELECT * FROM system_status WHERE id = 1').get();
    const questionCount = db.prepare('SELECT COUNT(*) as c FROM library_questions').get().c;
    const customQCount = db.prepare('SELECT COUNT(*) as c FROM custom_eval_questions').get().c;
    const periodStr = currentPeriods.map((p) => `  • ${p.period}: Auto=${p.self_start}→${p.self_end}, Supervisor=${p.supervisor_start}→${p.supervisor_end}, Feedback=${p.feedback_start}→${p.feedback_end}`).join('\n');
    ...
    if (config.can_manage_users)
        perms.push('✅ Gestionar Usuarios');
    if (config.can_manage_evaluations)
        perms.push('✅ Gestionar Evaluaciones');
    if (config.can_manage_vacations)
        perms.push('✅ Gestionar Vacaciones');
    if (config.can_manage_announcements)
        perms.push('✅ Gestionar Anuncios');
    if (config.can_manage_periods)
        perms.push('✅ Gestionar Periodos');
    if (config.can_manage_system)
        perms.push('✅ Gestionar Sistema');
    if (config.can_view_reports)
        perms.push('✅ Ver Reportes');
...
function getTools(config) {
    ...
    if (config.can_manage_users) {
        tools.push({
            ...
            execute: async (args) => {
                ...
                if (args.role === 'super_user') {
                    sql += ' AND is_super_user = 1';
                }
        ...
        tools.push({
            ...
            execute: async (args, _userId, _config) => {
                const { name, email, position, password, practice_area, is_admin, is_managing_partner } = args;
                if (!name || !email || !position || !password)
                    return JSON.stringify({ error: 'Nombre, email, posición y contraseña son obligatorios' });
                if (password.length < 6)
                    return JSON.stringify({ error: 'La contraseña debe tener al menos 6 caracteres' });
                const existing = db.prepare('SELECT id FROM users WHERE email = ?').get(email);
                if (existing)
                    return JSON.stringify({ error: 'El correo electrónico ya está registrado' });
                const id = uuidv4();
                const hashedPassword = await hashPassword(password);
                const hashedAnswer = ''; // Will be set on first login
                const now = new Date().toISOString();
                db.prepare(`INSERT INTO users (id, email, password_hash, security_question, security_answer, name, position, practice_area, is_admin, is_super_user, is_managing_partner, is_active, must_change_password, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, email, hashedPassword, '¿Cuál es su correo electrónico?', hashedAnswer, name, position, practice_area || null, (is_admin ? 1 : 0), 0, (is_managing_partner ? 1 : 0), 1, 1, now, now);
        ...
        tools.push({
            ...
            execute: async (args, _userId, _config) => {
                ...
                if (!Array.isArray(users))
                    return JSON.stringify({ error: 'users debe ser un arreglo' });
                const results = [];
                for (const u of users) {
                    const { name, email, position, password, practice_area, is_admin, is_managing_partner } = u;
                    if (!name || !email || !position || !password) {
                        results.push({ email, error: 'Campos obligatorios faltantes' });
                        continue;
                    }
                    if (password.length < 6) {
                        results.push({ email, error: 'Contraseña muy corta' });
                        continue;
                    }
                    const existing = db.prepare('SELECT id FROM users WHERE email = ?').get(email);
                    if (existing) {
                        results.push({ email, error: 'Email ya registrado' });
                        continue;
                    }
                    try {
                        const id = uuidv4();
                        ...
                        const now = new Date().toISOString();
                        db.prepare(`INSERT INTO users (id, email, password_hash, security_question, security_answer, name, position, practice_area, is_admin, is_super_user, is_managing_partner, is_active, must_change_password, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, email, hashedPassword, '¿Cuál es su correo electrónico?', '', name, position, practice_area || null, (is_admin ? 1 : 0), 0, (is_managing_partner ? 1 : 0), 1, 1, now, now);
        ...
        tools.push({
            ...
            execute: async (args) => {
                ...
                if (args.is_admin !== undefined) {
                    ...
                    values.push(args.is_admin ? 1 : 0);
                }
                if (args.is_managing_partner !== undefined) {
                    updates.push('is_managing_partner = ?');
                    values.push(args.is_managing_partner ? 1 : 0);
                }
                if (updates.length === 0)
                    return JSON.stringify({ error: 'No hay campos para actualizar' });
        ...
        tools.push({
            ...
            parameters: { type: 'object', properties: { user_id: { type: 'string', description: 'ID del usuario' } }, required: ['user_id'] },
            execute: async (args) => {
                const user = db.prepare('SELECT id, name, is_active FROM users WHERE id = ?').get(args.user_id);
                if (!user)
                    return JSON.stringify({ error: 'Usuario no encontrado' });
                if (!user.is_active)
                    return JSON.stringify({ error: 'El usuario ya está desactivado' });
        ...
        tools.push({
            ...
            execute: async (args) => {
                ...
                if (!user)
                    return JSON.stringify({ error: 'Usuario no encontrado' });
                if (user.is_active)
                    return JSON.stringify({ error: 'El usuario ya está activo' });
        ...
        tools.push({
            ...
            execute: async (args) => {
                ...
                const enriched = assignments.map(a => {
                    const emp = db.prepare('SELECT name FROM users WHERE id = ?').get(a.employee_id);
                    const sup = db.prepare('SELECT name FROM users WHERE id = ?').get(a.supervisor_id);
        ...
        tools.push({
            ...
            execute: async (args) => {
                ...
                if (!emp)
                    return JSON.stringify({ error: 'Empleado no encontrado' });
                if (!sup)
                    return JSON.stringify({ error: 'Supervisor no encontrado' });
                ...
                if (existing)
                    return JSON.stringify({ error: 'Esta asignación ya existe' });
                const id = uuidv4();
    ...
    if (config.can_manage_evaluations) {
        tools.push({
            ...
            parameters: { type: 'object', properties: { period: { type: 'string', description: 'Periodo (ej: 2026-H1)' } }, required: ['period'] },
            execute: async (args) => {
                const evals = db.prepare('SELECT * FROM evaluations WHERE period = ?').all(args.period);
        ...
        tools.push({
            ...
            execute: async (args) => {
                ...
                const evals = db.prepare(sql).all(...params);
                const user = db.prepare('SELECT name, position FROM users WHERE id = ?').get(args.user_id);
                if (!user)
                    return JSON.stringify({ error: 'Usuario no encontrado' });
                if (evals.length === 0)
                    return JSON.stringify({ user, message: 'No hay evaluaciones para este usuario en el periodo especificado' });
                ...
                if (evalIds.length > 0) {
                    ...
                    responses = db.prepare(`SELECT er.*, e.type as eval_type FROM evaluation_responses er JOIN evaluations e ON er.evaluation_id = e.id WHERE er.evaluation_id IN (${placeholders})`).all(...evalIds);
                }
                // Calculate scores by category
                const categories = {};
                const allQuestions = db.prepare('SELECT question_id, category, text FROM custom_eval_questions UNION ALL SELECT question_id, category, text FROM library_questions').all();
                ...
                for (const r of responses) {
                    ...
                    if (!categories[cat])
                        categories[cat] = { total: 0, count: 0, scores: [] };
        ...
        tools.push({
            ...
            execute: async (args, userId) => {
                ...
                if (!Array.isArray(questions))
                    return JSON.stringify({ error: 'questions debe ser un arreglo' });
                const results = [];
                ...
                for (const q of questions) {
                    if (!q.category || !q.text || !q.weight) {
                        results.push({ text: q.text, error: 'Campos faltantes' });
                        continue;
                    }
                    try {
                        const id = uuidv4();
                        ...
                        results.push({ text: q.text, error: String(e) });
                    }
                }
    ...
    if (config.can_manage_vacations) {
        ...
        tools.push({
            ...
            execute: async (args, userId) => {
                if (!['approved', 'rejected'].includes(args.action))
                ...
                if (!request)
                    return JSON.stringify({ error: 'Solicitud no encontrada' });
                db.prepare('UPDATE vacation_requests SET status = ? WHERE id = ?').run(args.action, args.request_id);
                db.prepare('INSERT INTO vacation_approvals (id, vacation_request_id, approver_id, approved_at, action, comment) VALUES (?, ?, ?, ?, ?, ?)').run(uuidv4(), args.request_id, userId, new Date().toISOString(), args.action, args.comment || null);
    ...
    if (config.can_manage_announcements) {
        ...
        tools.push({
            ...
            execute: async (args, userId) => {
                const id = uuidv4();
                const now = new Date().toISOString();
                db.prepare('INSERT INTO announcements (id, author_id, title, body, audience, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(id, userId, args.title, args.body, args.audience, now, args.expires_at || null);
    ...
    if (config.can_manage_periods) {
        tools.push({
            ...
            execute: async (args) => {
                ...
                if (existing)
                    return JSON.stringify({ error: 'El periodo ya existe' });
                db.prepare('INSERT INTO period_configs (period, self_start, self_end, supervisor_start, supervisor_end, feedback_start, feedback_end, action_plan_start, action_plan_end) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(args.period, args.self_start, args.self_end, args.supervisor_start, args.supervisor_end, args.feedback_start, args.feedback_end, args.action_plan_start, args.action_plan_end);
    ...
    if (config.can_view_reports) {
        tools.push({
            ...
            execute: async (args) => {
                ...
                const adminUsers = db.prepare('SELECT COUNT(*) as c FROM users WHERE is_admin = 1 AND is_active = 1').get().c;
                let evalStats = { total: 0, completed: 0 };
                if (args.period) {
                    const evals = db.prepare('SELECT * FROM evaluations WHERE period = ?').all(args.period);
                    evalStats = { total: evals.length, completed: evals.filter(e => e.completed_at).length };
                }
                const pendingVacations = db.prepare("SELECT COUNT(*) as c FROM vacation_requests WHERE status = 'pending'").get().c;
...
router.post('/chat', upload.single('file'), async (req, res) => {
    try {
        ...
        const history = db.prepare('SELECT role, content, tool_calls, tool_results FROM copilot_messages WHERE conversation_id = ? ORDER BY created_at ASC').all(convId);
        ...
        for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
            ...
            const messageToolCalls = assistantMessage.tool_calls;
            if (!messageToolCalls || messageToolCalls.length === 0) {
                finalResponse = assistantMessage.content || 'Lo siento, no pude generar una respuesta.';
                break;
            }
            // Save tool calls for display
            if (!toolCallsData)
                toolCallsData = JSON.stringify(messageToolCalls);
        ...
        db.prepare('INSERT INTO copilot_messages (id, conversation_id, role, content, tool_calls, tool_results, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(assistantMsgId, convId, 'assistant', finalResponse, toolCallsData, toolResultsData, new Date().toISOString());
